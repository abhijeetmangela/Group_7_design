...
So, $\frac{W}{S}$ comes out to be 59.5486. So taking our estimated weight the S comes out to be 1.0879.

Since we are not dropping any payload or using any fuel, our $\frac{W}{S}$ will remain the same overall. But in case of turning one wing can have a higher loading than other to balance out forces. But the overall $\frac{W}{S}$ will remain the same.

\subsubsection{Take off parameter}

According to \cite{Raymer.2006}, he states the the Take off Parameter is given as:- 

$$\text{TOP} = \frac{\frac{W}{S}}{ \sigma C_{L_{TO}} \left( \frac{P}{W} \right) }$$
where,
\begin{itemize}
    \item[-] $\sigma$ is the density ratio which we will take as 1.
    \item [-] $C_{L_{TO}}$ will be the Coefficient of lift at Take off 
\end{itemize}

We have taken the takeoff distance to be 80 m which comes out to be around 200 feet. Changing the previous equation :-

$$ \frac{W}{S} = \text{TOP} \sigma C_{L_{TO}} \left( \frac{P}{W} \right) $$

This equation will give us an estimate of the maximum $\frac{W}{S}$ which we can take for our aircraft.

The TOP is given in ref \cite{Raymer.2006} page number 130 in the form of a graph. The graph was digitalized using Image processing and a Linear regression was taken as 

\subsubsection{Landing distance}

According to \cite{Andersondesign} Anderson states 
